...
The **tidytext** package comes with a four existing sentiment lexicons or dictionaries, although note that you might be required to first install the **textdata** package and then authorise the lexicon downloads. These describe the emotional content of individual words in different formats, and have been put together manually. We will only be considering three of these.
...
(the resulting data has been included in the `data/` sub-directory, so there's no need to redo this step).
...
tidy_tweets <- tidy_tweets |> 
  ...
  mutate(
    sentiment = case_when(
      word == "trump" ~ "neutral",    # 'trump' is a positive word in the bing lexicon!
