...
# ================================================================
# NEURAL NETWORK-BASED COLLABORATIVE FILTERING
# Using Keras/TensorFlow for Deep Learning Approach
# ================================================================
...
prepare_nn_data <- function(user_item_matrix, test_ratio = 0.2, seed = 123) {
  ...
  # Create ID mappings with proper data types
  user_ids <- data.frame(
    user_index = as.integer(0:(n_users - 1)),
    user_id = rownames(user_item_matrix)
  )
  
  item_ids <- data.frame(
    item_index = as.integer(0:(n_items - 1)),
    item_id = colnames(user_item_matrix)
  )
...
evaluate_nn_model <- function(model, test_data) {
  
  # Prepare test data with proper data types (numpy arrays)
  x_test <- list(
    user_input = array(as.integer(test_data$user_index), dim = c(length(test_data$user_index), 1)),
    item_input = array(as.integer(test_data$item_index), dim = c(length(test_data$item_index), 1))
  )
  y_test <- as.numeric(test_data$rating)
...
```{r}
# ---------------------------------------------------------------
# 5. RECOMMEND FOR EXISTING USERS
# ---------------------------------------------------------------

recommend_for_user_nn <- function(model, user_item_matrix, user_id, 
                                 n_recommendations = 10, user_ids_map, item_ids_map) {
  ...
  # Create prediction data with proper data types (numpy arrays)
  x_pred <- list(
    user_input = array(as.integer(rep(user_idx, length(item_indices))), dim = c(length(item_indices), 1)),
    item_input = array(as.integer(item_indices), dim = c(length(item_indices), 1))
  )
  
  # Predict ratings
  pred_ratings <- model$predict(x_pred)
  pred_ratings <- as.vector(pred_ratings)
...
```{r}
# ---------------------------------------------------------------
# 6. RECOMMEND FOR NEW USERS (COLD START)
# ---------------------------------------------------------------

recommend_for_new_user_nn <- function(model, user_ratings, item_ids_map, 
                                     n_recommendations = 10) {
  ...
  # Create temporary user index (use max user index + 1 to avoid conflicts)
  temp_user_idx <- max(item_ids_map$item_index) + 1
  
  # Predict ratings for all items
  all_item_indices <- 0:(nrow(item_ids_map) - 1)
  x_pred <- list(
    user_input = array(as.integer(rep(temp_user_idx, length(all_item_indices))), dim = c(length(all_item_indices), 1)),
    item_input = array(as.integer(all_item_indices), dim = c(length(all_item_indices), 1))
  )
...
```{r}
# ---------------------------------------------------------------
# 7. CROSS-VALIDATION FOR NEURAL NETWORK
# ---------------------------------------------------------------
...
cross_validate_nn <- function(user_item_matrix, n_folds = 5, seed = 123,
  ...
  for (fold in 1:n_folds) {
    ...
    # Create train and test dataframes with proper data types
    train_data <- data.frame(
      user_index = as.integer(train_obs[, 1] - 1),
      item_index = as.integer(train_obs[, 2] - 1),
      rating = as.numeric(user_item_matrix[train_obs])
    ...
    # Evaluate
    results <- evaluate_nn_model(model_result$model, test_data)
...
```{r}
# ---------------------------------------------------------------
# 8. COMPLETE NEURAL NETWORK WORKFLOW
# ---------------------------------------------------------------
...
model_result <- train_nn_model(
  ...
  embedding_dim = 32,      # Smaller embedding for faster training
...
nn_model <- model_result$model

cat("\n=== STEP 3: Evaluate Model ===\n")
nn_results <- evaluate_nn_model(nn_model, nn_data$test)
cat("Neural Network RMSE:", round(nn_results$rmse, 3), "\n")
cat("Neural Network MAE:", round(nn_results$mae, 3), "\n")

cat("\n=== STEP 4: Neural Network Hyperparameter Tuning ===\n")
# Hyperparameter tuning for neural network
tune_nn_hyperparameters <- function(train_data, n_users, n_items, 
                                   embedding_dims = c(16, 32, 64),
                                   dense_units_list = c(32, 64, 128),
  ...
  total_combinations <- length(embedding_dims) * length(dense_units_list) * 
                       length(dropout_rates) * length(batch_sizes)
  
  cat("Testing", total_combinations, "hyperparameter combinations...\n")
    ...
    for (dense_units in dense_units_list) {
      for (dropout_rate in dropout_rates) {
        for (batch_size in batch_sizes) {
          ...
          cat("Combination", combination, "of", total_combinations, 
              "- emb_dim:", emb_dim, "dense:", dense_units, 
              "dropout:", dropout_rate, "batch:", batch_size, "\n")
          ...
          # Evaluate on validation set (using last 10% of training data)
          val_size <- floor(nrow(train_data) * 0.1)
          val_indices <- sample(nrow(train_data), val_size)
          val_data <- train_data[val_indices, ]
          
          val_results <- evaluate_nn_model(model_result$model, val_data)
          ...
          if (val_results$rmse < best_rmse) {
            ...
            cat("  -> New best RMSE:", round(best_rmse, 3), "\n")
          }
...
# Run hyperparameter tuning (with smaller parameter ranges for faster execution)
cat("Starting neural network hyperparameter tuning...\n")
tuning_results <- tune_nn_hyperparameters(
  train_data = nn_data$train,
  n_users = nn_data$n_users,
  n_items = nn_data$n_items,
  embedding_dims = c(16, 32),      # Smaller range for faster tuning
  dense_units_list = c(32, 64),    # Smaller range for faster tuning
  dropout_rates = c(0.2, 0.3),     # Smaller range for faster tuning
  batch_sizes = c(64, 128),        # Smaller range for faster tuning
...
cat("Best validation RMSE:", round(tuning_results$best_rmse, 3), "\n")

# Train final model with best parameters
cat("\nTraining final neural network model with optimal parameters...\n")
final_nn_model_result <- train_nn_model(
  train_data = nn_data$train,
  n_users = nn_data$n_users,
  n_items = nn_data$n_items,
  embedding_dim = tuning_results$best_params$embedding_dim,
  dense_units = tuning_results$best_params$dense_units,
  dropout_rate = tuning_results$best_params$dropout_rate,
  batch_size = tuning_results$best_params$batch_size,
  epochs = 30,  # More epochs for final model
  verbose = 1
)

final_nn_model <- final_nn_model_result$model

# Evaluate final model
final_nn_results <- evaluate_nn_model(final_nn_model, nn_data$test)
cat("Final neural network model performance:\n")
cat("RMSE:", round(final_nn_results$rmse, 3), "\n")
cat("MAE:", round(final_nn_results$mae, 3), "\n")

# Compare with initial model
cat("\nPerformance comparison:\n")
cat("Initial model RMSE:", round(nn_results$rmse, 3), "\n")
cat("Tuned model RMSE:", round(final_nn_results$rmse, 3), "\n")
improvement <- ((nn_results$rmse - final_nn_results$rmse) / nn_results$rmse) * 100
cat("Improvement:", round(improvement, 1), "%\n")
```

```{r}
# ---------------------------------------------------------------
# 9. NEURAL NETWORK RECOMMENDATIONS
# ---------------------------------------------------------------

# EXAMPLE 1: Recommend for existing user
cat("\n=== NEURAL NETWORK RECOMMENDATIONS FOR EXISTING USER ===\n")
sample_user <- rownames(user_item_matrix_nn)[1]
cat("User ID:", sample_user, "\n")

nn_recommendations_existing <- recommend_for_user_nn(
  model = final_nn_model,  # Use tuned model
  user_item_matrix = user_item_matrix_nn,
  user_id = sample_user,
  n_recommendations = 10,
  user_ids_map = nn_data$user_ids,
  item_ids_map = nn_data$item_ids
)

cat("Top 10 Neural Network recommendations:\n")
print(nn_recommendations_existing)
```

```{r}
# EXAMPLE 2: Recommend for new user (cold start problem)
cat("\n=== NEURAL NETWORK RECOMMENDATIONS FOR NEW USER (COLD START) ===\n")
...
nn_recommendations_new <- recommend_for_new_user_nn(
  model = final_nn_model,  # Use tuned model
  user_ratings = new_user_ratings,
  item_ids_map = nn_data$item_ids,
  n_recommendations = 10
...
cat("\nTop 10 Neural Network recommendations for new user:\n")
print(nn_recommendations_new)
```

```{r}
# ---------------------------------------------------------------
# 10. NEURAL NETWORK CROSS-VALIDATION (OPTIONAL)
# ---------------------------------------------------------------

# Run cross-validation (uncomment to run - takes significant time)
cat("\n=== NEURAL NETWORK CROSS-VALIDATION (OPTIONAL - UNCOMMENT TO RUN) ===\n")
