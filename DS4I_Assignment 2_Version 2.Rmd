...
cat("n_factors:", optimal_params_mf$min$dim, "\n")
cat("learning_rate:", optimal_params_mf$min$lrate, "\n")
cat("costp_l2:", optimal_params_mf$min$costp_l2, "\n")
cat("costq_l2:", optimal_params_mf$min$costq_l2, "\n\n")
...
# OPTION: For even faster hyperparameter tuning, use a smaller sample:
# set.seed(123)
# sample_users <- sample(rownames(user_item_matrix_nn), min(1000, nrow(user_item_matrix_nn)))
# user_item_matrix_nn <- user_item_matrix_nn[sample_users, ]

# Prepare data for hyperparameter tuning
h2o_data_tuning <- prepare_h2o_data(user_item_matrix_nn)

# SIMPLIFIED Parameter search space for faster tuning
activation_opt <- c("Rectifier", "Tanh")  # Only test 2 most common activations
hidden_opt <- list(c(64, 32), c(128, 64))  # Only test 2 proven architectures

l1_opt <- c(1e-5)  # Single value - regularization rarely needs extensive tuning
l2_opt <- c(1e-5)  # Single value - regularization rarely needs extensive tuning  
epochs_opt <- c(10)  # Single value - can increase later if needed
...
run_comprehensive_cv_comparison <- function(user_item_matrix, n_folds = 5, optimal_params_mf, optimal_params_nn) {
  ...
  ibcf_cv <- cross_validate_ibcf(user_item_matrix, n_folds = n_folds, k = 50)
  all_results[1, ] <- list(
    "Item-Based CF",
  ...
  ubcf_cv <- cross_validate_ubcf(user_item_matrix, n_folds = n_folds, k = 50)
  all_results[2, ] <- list(
    "User-Based CF",
  ...
  mf_cv <- cross_validate_mf(
    user_item_matrix, 
    n_folds = n_folds,
    n_factors = optimal_params_mf$mf$dim,  # Adjust based on your optimal_params structure
    learning_rate = optimal_params_mf$mf$lrate,
    costp_l2 = optimal_params_mf$mf$costp_l2,
    costq_l2 = optimal_params_mf$mf$costq_l2,
    n_iter = 50
  )
  all_results[3, ] <- list(
    "Matrix Factorization",
  ...
  # Neural Network - use the correct function name
  cat("\n=== Running Neural Network ===\n")
  nn_cv <- cross_validate_nn(
    user_item_matrix, 
    n_folds = n_folds,
    hidden = optimal_params_nn$hidden,
    epochs = optimal_params_nn$epochs,
  ...
  all_results[4, ] <- list(
    "Neural Network",
    round(mean(nn_cv$rmse), 3),
    round(sd(nn_cv$rmse), 3)
  )
...
cv_comparison_results <- run_comprehensive_cv_comparison(user_item_matrix, n_folds = 5, optimal_params_mf = optimal_params_mf, optimal_params_nn = optimal_params_nn)
