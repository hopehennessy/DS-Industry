...
```{r}
# ============================================================================
# DATASET SIZE IMPACT ANALYSIS - CORRECTED VERSION
# ============================================================================
# Research Question: How many book titles does a recommender system need
# to achieve good prediction accuracy?
# 
# Methodology:
# 1. For each catalog size (5, 10, 20, 30, 50, 75, 100, 145 books)
# 2. Select TOP N most popular books (ensures larger catalogs include smaller ones)
# 3. Split ratings 80/20, train MF model, evaluate RMSE
# 4. Run 5 times with different random seeds for robustness
# 5. Analyze sparsity impact and identify diminishing returns
#
# Key: Using top-N popular books ensures:
# - Sufficient data for each catalog size
# - Larger catalogs include all books from smaller catalogs
# - Results are comparable across catalog sizes
# ============================================================================

analyze_catalog_sizes_corrected <- function(data, 
                                           catalog_sizes = c(5, 10, 20, 30, 50, 75, 100, 145),
                                           optimal_params_mf,
                                           test_fraction = 0.2,
                                           n_seeds = 5,
  ...
  # Get book popularity rankings (used for all catalog sizes)
  book_popularity <- data %>%
    group_by(ISBN) %>%
    summarise(n_ratings = n(), .groups = "drop") %>%
    arrange(desc(n_ratings))
  
  cat("=== Dataset Size Impact Analysis ===\n")
  cat("Method: Within-catalog rating prediction using TOP N popular books\n")
  cat("Testing catalog sizes:", paste(catalog_sizes, collapse = ", "), "books\n")
  cat("Evaluation: ", n_seeds, " independent runs per catalog size\n\n")
  ...
  for (n_books in catalog_sizes) {
    ...
    cat("Selected", length(selected_books), "books,", 
        nrow(data_subset), "total ratings\n")
    ...
    for (seed_num in 1:n_seeds) {
      ...
      n_items <- ncol(user_item_matrix)
      sparsity <- mean(is.na(user_item_matrix)) * 100
      n_explicit <- sum(original_matrix[!is.na(original_matrix)] > 0)
      n_implicit <- sum(original_matrix[!is.na(original_matrix)] == 0)
      
      # Check if we have enough data
      if (n_ratings < 20) {
        cat(" SKIP (only", n_ratings, "ratings)\n")
        
        # Store result with NA RMSE
        all_results <- rbind(all_results, data.frame(
          ...
          n_test = NA,
          rmse = NA,
          reason = "Insufficient total ratings"
        ))
        next
      }
      ...
      # Create training matrices (hide test ratings)
      train_matrix <- user_item_matrix
      train_original <- original_matrix
      train_matrix[test_obs] <- NA
      train_original[test_obs] <- NA
      
      # Prepare data for MF (explicit ratings only)
      prepared <- prepare_recosystem_data(train_matrix, train_original)
      n_train_explicit <- nrow(prepared$train_data)
      ...
      for (i in 1:nrow(test_obs)) {
        user_idx <- test_obs[i, 1] - 1  # 0-indexed for recosystem
        item_idx <- test_obs[i, 2] - 1
        ...
        predictions[i] <- pmax(pmin(pred, 10), 1)  # Clip to [1, 10]
      }
      
      # Evaluate on EXPLICIT ratings only
      test_original <- original_matrix[test_obs]
      explicit_mask <- (test_original > 0)
      n_test_explicit <- sum(explicit_mask)
      ...
      if (n_test_explicit == 0) {
        ...
        all_results <- rbind(all_results, data.frame(
          ...
          n_test = 0,
          rmse = NA,
          reason = "No explicit test ratings"
        ))
        next
      }
      
      # Calculate RMSE on explicit ratings
      actual_explicit <- user_item_matrix[test_obs][explicit_mask]
      pred_explicit <- predictions[explicit_mask]
      rmse <- sqrt(mean((pred_explicit - actual_explicit)^2, na.rm = TRUE))
...
cat("Starting corrected catalog size analysis...\n\n")

catalog_analysis_results <- analyze_catalog_sizes_corrected(
  data = data,
  catalog_sizes = c(5, 10, 20, 30, 50, 75, 100, 145),
  optimal_params_mf = optimal_params_mf,
  test_fraction = 0.2,
...
kable(
  size_impact_results %>% 
    select(catalog_size, n_valid_runs, rmse_mean, rmse_se, sparsity_mean, 
           n_explicit_mean, n_train_mean, n_test_mean),
  caption = "Impact of Catalog Size on Prediction Accuracy (Corrected Analysis)",
  digits = 4,
  col.names = c("Catalog Size", "Valid Runs", "Mean RMSE", "SE", 
                "Sparsity %", "Explicit Ratings", "Train Size", "Test Size"),
  ...
  row_spec(which.min(size_impact_results$rmse_mean), 
           bold = TRUE, color = "white", background = "#4CAF50") %>%
  footnote(
    general = "TOP N popular books selected for each catalog size. Larger catalogs include all books from smaller catalogs. Results averaged over 5 independent runs.",
    general_title = "Note:"
  )
```
...
analyze_catalog_sizes_fair <- function(data, 
                                      ...
                                      optimal_params_mf,
                                      test_fraction = 0.2,
                                      min_user_ratings = 5,  # Minimum ratings per user to participate
                                      n_seeds = 5,
                                      base_seed = 123) {
...
cat("Starting fair catalog size analysis with per-user 80/20 split...\n\n")
...
catalog_analysis_results <- analyze_catalog_sizes_fair(
  ...
  optimal_params_mf = optimal_params_mf,
  test_fraction = 0.2,
  min_user_ratings = 5,  # Only include users with at least 5 ratings
  n_seeds = 5,
  base_seed = 123
)
...
kable(
  ...
  caption = "Impact of Catalog Size on Prediction Accuracy (Fair Per-User Split)",
  digits = 4,
  col.names = c("Catalog Size", "Valid Runs", "Mean RMSE", "SE", 
  ...
  row_spec(which.min(size_impact_results$rmse_mean), 
           bold = TRUE, color = "white", background = "#4CAF50") %>%
  footnote(
    general = "Per-user 80/20 split ensures fair comparison. TOP N popular books used for each catalog size. Only users with ≥5 ratings included.",
    general_title = "Note:"
  )
```
...
```{r dataset-size-visualization, fig.cap="Dataset Size Impact Analysis Results. Panel A shows RMSE trends across catalog sizes with error bars. Panel B displays sparsity patterns. Panel C reveals the relationship between sparsity and accuracy. Panel D shows individual seed variability.", fig.width=12, fig.height=10}
...
p2 <- ggplot(valid_results, aes(x = catalog_size, y = sparsity_mean)) +
  geom_line(color = "coral", size = 1.2) +
  geom_point(color = "coral", size = 3) +
  labs(title = "B. Matrix Sparsity by Catalog Size",
...
cat("=== DATASET SIZE IMPACT ANALYSIS: KEY FINDINGS ===\n\n")
...
kable(recommendations, 
      caption = "Practical Recommendations for Catalog Size",
...
cat("✓ Dataset size analysis complete!\n")
