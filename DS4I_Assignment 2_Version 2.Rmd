...
```{r shared-functions}
# ================================================================
# SHARED FUNCTIONS (Used by both UBCF and IBCF)
# ================================================================
...
cross_validate_ibcf <- function(user_item_matrix, n_folds = 5, k = 30) {
  ...
  cat("Starting", n_folds, "-fold cross-validation for IBCF...\n")
  ...
  for (fold in 1:n_folds) {
    ...
    for (i in 1:nrow(test_obs)) {
      ...
      # Get similarities and apply k-NN
...
cross_validate_h2o <- function(user_item_matrix, n_folds = 5, seed = 123, optimal_params) {
...
# Cross-Validation Evaluation
nn_cv_results <- cross_validate_h2o(user_item_matrix, n_folds = 5, optimal_params = optimal_params)
...
# Function to recommend for existing users (Neural Network)
...
# Function to recommend for new users (cold start problem - Neural Network)
recommend_for_new_user_nn <- function(model, user_ratings, book_ids,
...
recommendations_existing <- recommend_for_user_nn(
  model = model,
  user_item_matrix = user_item_matrix,
  user_id = sample_user,
  n_recommendations = 10,
...
# Get recommendations for new user
recommendations_new <- recommend_for_new_user_nn(
  model = model,
  user_ratings = new_user_ratings,
  book_ids = prepared$book_ids,
  n_recommendations = 10,
