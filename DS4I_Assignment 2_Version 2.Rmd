...
evaluate_predictions <- function(pred, actual) {
...
cross_validate_ubcf <- function(user_item_matrix, n_folds = 5, k = 30) {
  ...
  for (fold in 1:n_folds) {
    ...
    # Calculate metrics
    rmse_fold <- evaluate_predictions(predictions, actual)
    
    cv_results[fold, ] <- list(fold, rmse_fold)
...
evaluate_mf_model <- function(model, test_data) {
  ...
  rmse_value <- evaluate_predictions(predictions, actual_ratings)
...
# Recommend for new users (cold start)
recommend_for_new_user_mf <- function(model, new_user_ratings, user_item_matrix, 
  ...
  rated_indices <- item_ids_map$item_index[match(ratings_df$item_id, item_ids_map$item_id)]
  temp_user_idx <- nrow(user_item_matrix)
  unrated_indices <- setdiff(0:(nrow(item_ids_map)-1), rated_indices)
  
  if (length(unrated_indices) == 0) return(data.frame())
  
  pred_set <- data_memory(user_index = rep(temp_user_idx, length(unrated_indices)),
                          item_index = unrated_indices, index1 = FALSE)
  pred_ratings <- model$predict(pred_set, out_memory())
  pred_ratings <- pmax(pmin(pred_ratings, 10), 1)
...
cross_validate_mf_recosystem <- function(user_item_matrix, 
                                         optimal_params_mf,
                                         n_folds = 5, 
                                         n_iter = 50, 
                                         seed = 123, 
  ...
  # Get observed ratings (non-missing entries)
  observed <- which(!is.na(user_item_matrix), arr.ind = TRUE)
  n_ratings <- nrow(observed)
  ...
  for (fold in 1:n_folds) {
    ...
    train_data <- data.frame(
      user_index = train_obs[, 1] - 1,   # 0-based indexing
    ...
    # Convert to recosystem data source
    train_set <- data_memory(
      user_index = train_data$user_index,
      item_index = train_data$item_index,
      rating = train_data$rating,
    ...
    # Calculate RMSE using evaluate_predictions function
    rmse_fold <- evaluate_predictions(preds, test_data$rating)
    cv_results[fold, ] <- c(fold, rmse_fold)
...
```{r}
# Run CV with optimal parameters from tuning
cv_results_mf <- cross_validate_mf_recosystem(
...
cv_results_mf %>%
  knitr::kable(caption = "Matrix Factorization Cross-Validation Results", digits = 4) %>%
...
# New user
new_user_ratings <- setNames(c(8, 9, 7, 6, 8), colnames(user_item_matrix_mf)[1:5])
...
kable(head(top_models_df, 5), caption = "Top 5 Neural Network Models (H2O Grid Search)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
kable(best_nn_results, caption = "Best Neural Network Model Specifications") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```
...
evaluate_h2o_model <- function(model, test_data, user_means) {
  ...
  predictions <- as.data.frame(h2o.predict(model, test_data))
  test_data_df <- as.data.frame(test_data)
  
  # Denormalize
  user_ids_for_test <- as.character(test_data_df$user_id)
  user_means_for_test <- user_means[user_ids_for_test]
  ...
  actual <- test_data_df$rating
  rmse_value <- evaluate_predictions(predictions_df$predict, actual)
  
  return(list(rmse = rmse_value, predictions = predictions_df$predict, actual = actual))
}
```
...
optimal_params_nn <- list(
  ...
  activation = best_model@allparameters$activation,
  hidden_dropout_ratios = ifelse(is.null(best_model@allparameters$hidden_dropout_ratios), list(NULL), best_model@allparameters$hidden_dropout_ratios),
...
model_nn <- train_h2o_model(
  ...
  hidden_dropout_ratios = optimal_params_nn$hidden_dropout_ratios[[1]],
...
cross_validate_nn <- function(user_item_matrix, n_folds = 5, seed = 123, 
  ...
  for (fold in 1:n_folds) {
    ...
    # Create test data with original ratings (for proper evaluation)
    test_data <- data.frame(
    ...
    # Evaluate (with proper denormalization using training user means)
    results <- evaluate_h2o_model(model, test_h2o, train_user_means)
...
# Prepare data for final model (using FULL dataset, not the sample)
prepared <- prepare_h2o_data(user_item_matrix_nn)
...
run_comprehensive_cv_comparison <- function(user_item_matrix, n_folds = 5, optimal_params_mf, optimal_params_nn, k = 50) {
  ...
  # User-Based CF
  ubcf_cv <- cross_validate_ubcf(user_item_matrix, n_folds = n_folds, k = k)
  all_results[2, ] <- list(
    "User-Based CF",
  ...
  # Item-Based CF
  ibcf_cv <- cross_validate_ibcf(user_item_matrix, n_folds = n_folds, k = k)
  all_results[1, ] <- list(
    "Item-Based CF",
...
# User-item matrix for fair comparison across all methods

unified_matrix <- create_user_item_matrix(
  data,
  min_ratings_per_book = 5,  
  min_ratings_per_user = 3   
)
...
cat("Unified matrix dimensions:", nrow(unified_matrix), "users x", ncol(unified_matrix), "items\n")
cat("Matrix sparsity:", round(mean(is.na(unified_matrix)) * 100, 2), "%\n\n")

# Ensure optimal parameters are available for comparison
# If not already defined from previous chunks, create default values
...
# Run the comprehensive comparison using the unified matrix
cv_comparison_results <- run_comprehensive_cv_comparison(
  user_item_matrix = unified_matrix,  
  n_folds = 5, 
...
if (exists("cv_comparison_results") && exists("dataset_size_results") && nrow(dataset_size_results) > 0) {
  
  cat("\n=== DIMINISHING RETURNS ANALYSIS ===\n")
  cat("Determining if there's a point where adding more books doesn't improve accuracy\n\n")
  
  # Identify the best performing method from cross-validation results
  ...
  # Analyze only the best method for diminishing returns
  method_data <- dataset_size_results %>%
    filter(Method == best_method) %>%
    arrange(Dataset_Size)
  
  if (nrow(method_data) >= 3) {
    cat("--- Diminishing Returns Analysis for", best_method, "---\n")
    ...
    if (!is.na(diminishing_point)) {
      ...
      cat("→ Optimal catalog size for", best_method, ":", optimal_size, "books\n\n")
    } else {
      cat("→ No clear diminishing returns point found\n\n")
    }
    
    # Create visualization for best method only
    p_size <- ggplot(method_data, aes(x = Dataset_Size, y = RMSE)) +
      geom_line(size = 1.2, color = "steelblue") +
      geom_point(size = 3, color = "steelblue") +
      labs(title = paste("Predictive Accuracy vs Dataset Size -", best_method),
      ...
      scale_x_continuous(breaks = unique(method_data$Dataset_Size))
    
    print(p_size)
    
  } else {
    cat("Insufficient data points for diminishing returns analysis\n")
  }
  
} else {
  cat("Cross-validation or dataset size results not available for diminishing returns analysis\n")
}
```
  ...
  analyze_dataset_size_impact_best <- function(data, book_sizes = c(30, 60, 90, 120, 150), 
    ...
    for (n_books in book_sizes) {
      ...
      if (best_method == "Item-Based CF") {
        ...
        cv_results <- cross_validate_nn(subset_matrix, n_folds = n_folds, 
                                       hidden = optimal_params_nn$hidden,
                                       epochs = optimal_params_nn$epochs,
                                       activation = optimal_params_nn$activation,
                                       hidden_dropout_ratios = optimal_params_nn$hidden_dropout_ratios,
