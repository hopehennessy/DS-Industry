...
Sample Data for Experimental Validation
...
evaluate_predictions <- function(pred, actual) {
  ...
  rmse_sd <- sd(pred - actual, na.rm = TRUE)
  return(list(rmse = rmse, rmse_sd = rmse_sd))
}
...
# Use unified matrix for consistency across all methods
user_item_matrix_ubcf <- user_item_matrix
...
# Use unified matrix for consistency across all methods
user_item_matrix_ibcf <- user_item_matrix
...
# Use unified matrix for consistency across all methods
user_item_matrix_mf <- user_item_matrix
...
cross_validate_mf_recosystem <- function(user_item_matrix, 
  ...
  for (fold in 1:n_folds) {
    ...
    metrics <- evaluate_predictions(preds, test_data$rating)
    cv_results[fold, ] <- c(fold, metrics$rmse)
...
# Use unified matrix for consistency across all methods
user_item_matrix_nn <- user_item_matrix
...
cross_validate_nn <- function(user_item_matrix, n_folds = 5, seed = 123, 
  ...
  for (fold in 1:n_folds) {
    ...
    test_data_df <- as.data.frame(test_h2o)
    
    # Denormalize
    user_ids_for_test <- as.character(test_data_df$user_id)
    user_means_for_test <- train_user_means[user_ids_for_test]
    ...
    metrics <- evaluate_predictions(predictions_df$predict, actual)
...
# Prepare data for final model using unified matrix
prepared <- prepare_h2o_data(user_item_matrix_nn)
...
# Use the unified matrix created earlier for fair comparison across all methods
